...
if ( ! function_exists( 'proplay_hybridauth_login_handler' ) ) :

function socialplay_hybridauth_login_handler()
{
    ...
    $plugin_dir = get_template_directory() . '/includes/thirdparty/wordpress-social-login/';
    $plugin_url = get_template_directory_uri() . '/includes/thirdparty/wordpress-social-login/';
    ...
    require_once  $plugin_dir . "wp-social-login.php";

    if ( ! get_option( 'proplay-hybridauth-installed' ) ) {
        wsl_install();

        update_option( 'proplay-hybridauth-installed', 1 );
    }
}

endif;

socialplay_hybridauth_login_handler();
            ...
            <div class="chan-box">
                <div class="channel">
                    ...
                    <div class="chan-control" data-id="<?php echo kvalue($chan, 'term_id'); ?>">
                        ...
                        <?php endif; ?>
                        <span><?php echo kvalue($chan, 'count');?> <?php echo ($type == 'album') ? __('Audios', 'exc-proplay-theme') : __('Videos', 'exc-proplay-theme'); ?></span>
                    </div>
                </div><!-- channel ends -->
            </div>
        <?php endif;?>
...
function fw_comment_form( $args = array(), $post_id = null )
{
    ...
    $author_val = __('Enter Name', 'exc-proplay-theme');
    $email_val = __('Enter Email', 'exc-proplay-theme');
    $url_val = __('Enter Website', 'exc-proplay-theme');
    $comment_val = __('Enter Comment', 'exc-proplay-theme');
    ...
    $required_text = sprintf( ' ' . __('Required fields are marked %s', 'exc-proplay-theme'), '<span class="required">*</span>' );
    $defaults = array(
        ...
        'must_log_in' => '<p>' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.','exc-proplay-theme' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
        'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>','exc-proplay-theme' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
        'comment_notes_before' => '<p class="comment-notes">' . __( 'Your email address will not be published.','exc-proplay-theme' ) . ( $req ? $required_text : '' ) . '</p>',
        'comment_notes_after'  => '<p>' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s','exc-proplay-theme' ), '<p><code>' . allowed_tags() . '</code></p>' ) . '</p>',
        'id_form' => 'commentform',
        'id_submit' => 'submit',
        'title_reply' => __( 'Leave a Comment', 'exc-proplay-theme' ),
        'title_reply_to' => __( 'Leave a Reply to %s', 'exc-proplay-theme' ),
        'cancel_reply_link' => __( 'Cancel reply', 'exc-proplay-theme' ),
        'label_submit' => __( 'Post', 'exc-proplay-theme' ),
    );
            ...
            <div class="comment-box" id="comment-<?php comment_ID();?>">
                ...
                <div class="comment-tp">
                    ...
                    if( $comment->comment_approved == '0' ) : ?>
                        <em><?php _e( 'Your comment is awaiting moderation.', 'exc-proplay-theme' ); ?></em>
                    <?php endif; ?>
                    <strong><?php echo get_comment_author_link(); ?></strong>
                    <span><?php comment_date( get_option( 'date_format' ) ); _e(' at ', 'exc-proplay-theme'); comment_time( 'g:i a' ); /** print date time */?></span>
                    <ul class="reply">
...
function fw_video_row( $query, $prefix = 'video' )
{
    ...
    while( $query->have_posts() ): $query->the_post();
        ...
        if ( ! $thumb && kvalue( $video, 'thumb') ) {
            $thumb[0] = kvalue( $video, 'thumb');
        } elseif( !$thumb ) {
            $thumb[0] = get_template_directory_uri() .'/images/video-large.gif';
        }

        $source = ( kvalue($video, 'source') == 'local' ) ? 'local-' . $prefix : kvalue( $video, 'source' );

        $pending = ( $post->post_status == 'pending' ) ? '<span><i>' . __('Pending', 'exc-proplay-theme') . '</i></span>' : '';
        ...
        $content = '<div class="vid-box large" style="visibility:visible;">
                        <div class="video-thumb video-thumb-gray">
                            ...
                            <div class="vid-bot" data-id="' . get_the_ID() . '">
                                <a href="javascript:void(0);" class="edit_' . $prefix . '">' . __('Edit ', 'exc-proplay-theme') . ucwords( $prefix ) . '</a>
                                <a href="javascript:void(0);" class="del_' . $prefix . '">' . __('Delete ', 'exc-proplay-theme') . ucwords( $prefix ) . '</a>
                            </div>
                        </div>
                    </div>';
        ...
        $term_key = ( $terms ) ? $name : __('Uncategorized', 'exc-proplay-theme');
   ...
   <?php if( kvalue($settings, 'msg_status') == 'on'): ?>
    <div class="alert">
        ...
        <?php if( kvalue($settings, 'msg_button') == 'on' && !is_user_logged_in() ): ?>
            ...
            <?php if($signup_obj ): ?>
                <a class="btn btn-red" href="<?php echo get_permalink( kvalue( $signup_obj, 'ID') ); ?>"><?php _e('SignUp', 'exc-proplay-theme'); ?></a>
            <?php endif; ?>
        <?php endif; ?>
    </div>
    <?php endif;
...
function fw_news_ticker()
{
    ...
    <?php if( kvalue( $settings, 'ticker_status') == 'on'): ?>
        ...
        <div class="news-strip"> <strong><?php _e('Latest News :', 'exc-proplay-theme'); ?></strong>
            <?php if( $query ): ?>
            <ul id="ticker">
...
function fw_user_posts_count($author, $post_type = 'post')
{
    ...
    if ( $post_count ) {
        ...
        return __('Not Found', 'exc-proplay-theme');
    }
}
...
function get_video_meta($id, $post_type = 'wpnukes_videos')
{
    ...
    $contnet = get_post_meta( $id, '_wpnukes_' . $type, true );
...
function fw_prep_video_query()
{
    ...
    elseif(kvalue($settings, 'video_sortby') == 'title')
    {
        ...
        $query = array_merge( array( 'orderby' => 'ID', 'order' => 'DESC' ), $query);
    }
...
function fw_the_pagination($args = array(), $echo = 1)
{
    ...
    $default =  array('base' => str_replace( 99999, '%#%', esc_url( get_pagenum_link( 99999 ) ) ), 'format' => '?paged=%#%', 'current' => max( 1, get_query_var('paged') ),
                        'total' => $wp_query->max_num_pages, 'next_text' => __('Next', 'exc-proplay-theme'), 'prev_text' => __('Previous', 'exc-proplay-theme'));
...
function fw_get_the_categories()
{
    ...
    if($categories){
        foreach($categories as $category) {
            $output .= '<a class="btn btn-cyan" href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "View all posts in %s", 'exc-proplay-theme' ), $category->name ) ) . '">'.$category->cat_name.'</a>'.$separator;
        }
...
function fw_contact_form_submit()
{
    ...
    $t->validation->set_rules('contact_name','<strong>'.__('Name', 'exc-proplay-theme').'</strong>', 'required|min_length[4]|max_lenth[30]');
    $t->validation->set_rules('contact_email','<strong>'.__('Email', 'exc-proplay-theme').'</strong>', 'required|valid_email');
    $t->validation->set_rules('contact_subject','<strong>'.__('Subject', 'exc-proplay-theme').'</strong>', 'required|min_length[5]');
    $t->validation->set_rules('contact_message','<strong>'.__('Message', 'exc-proplay-theme').'</strong>', 'required|min_length[5]');
    ...
    if( kvalue($settings, 'captcha_status') == 'on')
    {
        ...
        {
            $t->validation->_error_array['captcha'] = __('Invalid captcha entered, please try again.', 'exc-proplay-theme');
        }
    }
    ...
    if($t->validation->run() !== FALSE && empty($t->validation->_error_array))
    {
        ...
        $message = kvalue($settings, 'success_msg') ? $settings['success_msg'] : sprintf( __('Thank you <strong>%s</strong> for using our contact form! Your email was successfully sent and we will be in touch with you soon.','exc-proplay-theme'), $name);

        $messages = '<div class="alert alert-success">
                        <p class="title">'.__('SUCCESS! ', 'exc-proplay-theme').$message.'</p>
                    </div>';
         ...
         if( is_array( $t->validation->_error_array ) )
         {
             foreach( $t->validation->_error_array as $msg )
             {
                 $messages .= '<div class="alert alert-error">
                                    <p>'.__('Error! ', 'exc-proplay-theme').$msg.'</p>
                                </div>';
             }
         }
    }
...
{
    $array = array( 'tpl-channels.php' => __('Channels', 'exc-proplay-theme'), 'contact.php' => __('Contact Us', 'exc-proplay-theme'), 'forgot.php' => __('Forgot Password', 'exc-proplay-theme'),
            'front-page-tpl.php' => __('Homepage', 'exc-proplay-theme'), 'login.php' => __('Login', 'exc-proplay-theme'), 'profile.php' => __('Profile', 'exc-proplay-theme'), 'signup.php' => __('Signup', 'exc-proplay-theme'),
            'tpl-albums.php' => __('Albums', 'exc-proplay-theme'), 'tpl-playlists.php' => __('Playlists', 'exc-proplay-theme') );
...
function new_excerpt_more($more) {
    ...
    return '<a class="moretag" href="'. get_permalink($post->ID) . '"> '.__('Read more..', 'exc-proplay-theme').'</a>';
}
