...
function fw_enqueue_scripts()
{
    ...
    $scripts = array('proplay-login'=>'login.js', 'jquery-ui-tooltip'=>'jquery-ui-tooltip.js', 'jquery-superslides'=>'jquery.superslides.js', 'jquery-jcarousel-min'=>'jquery.jcarousel.min.js',
    ...
    {
        wp_enqueue_script( array('proplay-login', 'comment-reply', 'jquery-jcarousel-min','jquery-prettyPhoto' ) );
    }
    ...
    wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css', array('main-styles')  );
        ...
        jQuery(document).ready(function($) {
            ...
            if( $GLOBALS['_webnukes']->fw_get_settings('sub_general_settings', 'play_video') == 'on' ):?>
                ...
                $('.icon-play').live('click', function(e){
                    var videoSelector = $(this).parents('.video-thumb').children( 'iframe, video, .wp-video' );
                    var imagSelector = $(this).parents('.video-thumb').children( 'img, .thumb-info, .thumb-hover' );
                    if( videoSelector.length ) {
                        $(videoSelector).css('display','block');
                        $(imagSelector).css('display','block');
...
function fw_demo_color_picker()
{
    ...
    $colorpicker_l10n = array(
        'clear' => __( 'Clear', 'exc-proplay-theme' ),
        'defaultString' => __( 'Default', 'exc-proplay-theme' ),
        'pick' => __( 'Select Color', 'exc-proplay-theme' )
    );
