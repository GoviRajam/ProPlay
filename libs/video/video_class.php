...
class WPNukes_Helper
{
    ...
    function __construct()
    {
        ...
        add_action('wpnukes_submit_user_video', array($this, 'update_user_video'));
    ...
    function create_term($data)
    {
        ...
        elseif( !is_wp_error($term) )
        {
            ...
            $this->push_msg('<p>'.sprintf(__(' %s is Succefully Created', 'exc-socialplay-theme'), kvalue($data, 'title' )).'</p>');
        ...
        switch( $type)
        {
            ...
            case 'local':
                ...
                if( strstr( kvalue( $meta, 'mime_type'), 'audio/' ) ){
                    ...
                    return do_shortcode('[video '.kvalue( $meta, 'fileformat').'="'.wp_get_attachment_url(kvalue( $array, 'attachment_id' )).'" width="555" height="252" poster="'.kvalue($thumb, 0).'"][/video]');
                }
                return '';
            break;
        }
    ...
    function add_videos($data)
    {
        ...
        {
            $type = __('Audio file', 'exc-proplay-theme');
            $post_type = 'wpnukes_audios';
        }
        elseif( kvalue( $data, 'add_video'))
        {
            $type = __('Video file', 'exc-proplay-theme');
        ...
        if( $link ) {
            ...
            {
                $success = false;
                $is_playlist = ( count( $result ) > 1 ) ? true : false;
                ...
                foreach( $result as $res )
                {
                    ...
                    $success = $this->update_user_vide( $combined, true );
            ...
            if ( $is_playlist )
            {
                if ( $stats['success'] )
                {
                    $this->push_msg( sprintf( __('The %d new file\'s are successfully added and pending for review.', 'exc-proplay-theme'), $stats['success'] ), 'success');
                } elseif ( ! empty( $duplicate ) )
                {
                    if ( count( $duplicate ) > 3 )
                    {
                        $this->push_msg( __( 'The files in this link are already processed.', 'exc-proplay-theme' ), 'error' );
                    } else
                    {
                        $this->push_msg( implode( "<br />", $duplicate ), 'error' );
                    }

                } else
                {
                    $this->push_msg( __('We are unable to process your request, please try again or check the URL you entered.', 'exc-proplay-theme'), 'error');
                }

            } else
            {
                if ( $stats['success'] && $def_status == 'pending' )
                {
                    $this->push_msg( __('The file has been submitted successfully and pending for review.', 'exc-proplay-theme' ), 'success');
                } elseif ( $stats['success'] && $def_status == 'publish' )
                {
                    $this->push_msg( __('The file has been submitted successfully.', 'exc-proplay-theme' ), 'success');
                } elseif ( $stats['success'] && $def_status == 'draft' )
                {
                    $this->push_msg( __('The file has been submitted successfully and save to draft for review.', 'exc-proplay-theme' ), 'success');
                } else if( ! empty( $duplicate ) )
                {
                    $this->push_msg( kvalue( $duplicate, 0 ), 'error' );
                } else
                {
                    $this->push_msg( __('We are unable to process your request, please try again or check the URL you entered.', 'exc-proplay-theme'), 'error');
                }
    ...
    function update_user_vide( $data, $uploading = false )
    {
        ...
        if ( $uploading && $this->prefix && isset( $data[ $this->prefix . 'id'] ) )
        {
            $args = array(
                    'meta_key'          => '_' . str_replace( 'webnukes', 'wpnukes', $this->prefix ) . 'id',
                    'meta_value'        => $data[ $this->prefix . 'id' ],
            ...
            if ( $the_query->have_posts() )
            {
                ...
                while ( $the_query->have_posts() ) {
                    ...
                    $duplicate = '<p>' . sprintf(
                                                __('This link is already processed %s ', 'exc-proplay-theme'),
        ...
        {
            $this->push_msg( '<p>'.__('You are not authorized to Manage videos/audios', 'exc-proplay-theme').'</p>', 'success');
            wp_redirect(get_permalink());
            exit;
        }
        ...
        $post = array(
          ...
          'post_content'   => kvalue( $data, 'webnukes_video_desc' ), //The full text of the post.
          ...
          'post_status'    => $def_status, //Set the status of the new post.
        ...
        if( ! is_wp_error($res) )
        {
            ...
            $type = ($post_type == 'wpnukes_audios') ? __('Audio', 'exc-proplay-theme') : __('Video', 'exc-proplay-theme');

            if ( ! $uploading )
            {
                $this->push_msg( '<p>'.__('The file has been saved successfully.', 'exc-proplay-theme').'</p>', 'success');
                wp_redirect( get_permalink() );
            }
    ...
    function update_user_profile( $data )
    {
        ...
        if ( $pass = kvalue( $data, 'pass1' ) ) {
            if ( $pass == kvalue( $data, 'pass2' ) ) {
                $data['user_pass'] = $pass;
            } else {
                $this->push_msg(__('Both passwords do not match, Retry again', 'pro-play' ), 'error');
            }
        }
        ...
        if ( $file = kvalue( $_FILES, 'avatar_file') ) {
            ...
            if ( kvalue( $file, 'name') ) {
                ...
                if ( $avatar )  {
                    update_user_meta( kvalue( $data, 'ID' ), 'avatar', kvalue( $avatar, 'url' ) );
                } else {
                 $this->push_msg( __('Failed to updload profile image', 'pro-play' ), 'error' );
                }
            } else {

                $url = kvalue( $_POST, "avatar" );
                if ( $url && ! $_webnukes->validation->valid_url( $url ) ) {
                    $this->push_msg( __('Avatar Field Must Contain a valid url', 'pro-play' ), 'error' );
                } else {
                    update_user_meta( kvalue( $data, 'ID' ), 'avatar', $url );
                }
        ...
        foreach( $fields as $k=>$v ) {
            if ( kvalue( $v, 'validation' ) ) {
                $_webnukes->validation->set_rules( $k, kvalue( $v, 'label' ), kvalue( $v, 'validation' ) );
            }
        }
        ...
        if ( $_webnukes->validation->run() !== false ) {
            ...
            if ( ! is_wp_error( $update ) ) {
                $this->push_msg(__('Profile is updated successfully', 'pro-play' ) );
            } else {
                $this->push_msg(__('Failed to update profile', 'pro-play' ), 'error' );
            }
        } else {
    ...
    function _save_html5_thumb($src, $name) {
        ...
        if ( true ) {
            ...
            $posterfile = texttoslug( $name ) . '_thumb';
