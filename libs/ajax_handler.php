...
class Ajax_Handler
{
    ...
    function fb_login()
    {
        ...
        $existing = get_user_by( 'name', kvalue( $_POST, 'email' ) );
        ...
        if( !isset($existing->ID))
        {
            ...
            if( !is_wp_error($user_id))
            {
                ...
                $message = sprintf( __("Your new user is created, Details are \nusername is: \t %s \npassword is: \t %s", 'exc-proplay-theme'), kvalue($_POST, $username), $password );
    ...
    function generate_form()
    {
        ...
        {
            $data = array('heading'=> __('File Uploader', 'exc-socialplay-theme'));
    ...
    function upload_video($uploading = 'video')
    {
        ...
        if($uploading == 'audio' && $post_type != 'wpnukes_audios') exit( __('Please upload a valid audio file.', 'exc-proplay-theme'));
        elseif($uploading == 'video' && $post_type == 'wpnukes_audios') exit( __('Please upload a valid video file.', 'exc-proplay-theme'));

        $_REQUEST['type'] = ( $post_type == 'wpnukes_videos' ) ? 'edit_video' : 'edit_audio';

        if( !$post_type ) exit( __('Invalid file format', 'exc-proplay-theme'));
        ...
        if ( $movefile )
        {
            ...
            if( $post_type == 'wpnukes_videos' )
            {
                echo '<div class="video-capture-container">
                        ...
                        <button class="btn-red btn capture-video">'.__('Capture', 'exc-socialplay-theme').'</button>
                        <span class="images">'.__('<p>Click the button "Capture" to capture the image of the video</p>', 'exc-proplay-theme').'</span>
                    </div>';
            }
